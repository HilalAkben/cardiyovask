...
class ComparativeAnalyzer:
    ...
    def run_xgboost_only_analysis(self, data_with_outliers, data_without_outliers):
        """Sadece XGBoost modeli için analiz çalıştır."""
        print("="*80)
        print("KALP KRİZİ RİSK TAHMİN MODELİ - XGBOOST ANALİZ")
        print("="*80)
        
        # 1. Outlier'lı verilerle XGBoost analizi
        print("\n" + "="*40)
        print("1. OUTLIER'LAR İLE XGBOOST ANALİZİ")
        print("="*40)
        
        analyzer_with_outliers = XGBoostOnlyAnalyzer()
        self.outlier_results = analyzer_with_outliers.run_xgboost_analysis(
            data_with_outliers['X_train'],
            data_with_outliers['X_test'],
            data_with_outliers['y_train'],
            data_with_outliers['y_test'],
        ...
        # 2. Outlier'lar çıkarılarak XGBoost analizi
        print("\n" + "="*40)
        print("2. OUTLIER'LAR ÇIKARILARAK XGBOOST ANALİZİ")
        print("="*40)
        
        analyzer_without_outliers = XGBoostOnlyAnalyzer()
        self.no_outlier_results = analyzer_without_outliers.run_xgboost_analysis(
            data_without_outliers['X_train'],
            data_without_outliers['X_test'],
            data_without_outliers['y_train'],
            data_without_outliers['y_test'],
        ...
        # 3. Karşılaştırmalı sonuçlar
        self.plot_xgboost_comparative_results()
        
        return self.outlier_results, self.no_outlier_results
    
    def plot_xgboost_comparative_results(self):
        """XGBoost karşılaştırmalı sonuçlarını görselleştir."""
        print("\n" + "="*40)
        print("3. XGBOOST KARŞILAŞTIRMALI SONUÇLAR")
        print("="*40)
        ...
        for idx, metric in enumerate(metrics):
            if idx < len(axes):
                ...
                axes[idx].set_title(f'XGBoost {metric} Karşılaştırması')
        ...
        plt.tight_layout()
        plt.savefig('xgb_comparative_analysis.png', dpi=300, bbox_inches='tight')
        plt.show()
        
        # Sonuçları yazdır
        print("\n=== XGBOOST KARŞILAŞTIRMALI SONUÇLAR TABLOSU ===")
        print(metrics_comparison.to_string(index=False))
        
        # En iyi modelleri karşılaştır
        print(f"\n=== XGBOOST EN İYİ MODEL KARŞILAŞTIRMASI ===")
...
class XGBoostOnlyAnalyzer:
    """Sadece XGBoost modeli için analiz sınıfı."""
    
    def __init__(self):
        if not XGBOOST_AVAILABLE:
            raise ImportError("XGBoost bulunamadı. Lütfen 'pip install xgboost' komutunu çalıştırın.")
    ...
    def evaluate_xgboost(self, X_test, y_test):
        """XGBoost modelini değerlendir ve metrikleri hesapla."""
        print("\n=== XGBOOST MODEL DEĞERLENDİRMESİ ===")
        ...
        # En iyi model olarak ayarla
        self.best_model = self.model
        self.best_model_name = 'XGBoost'
        self.best_score = f1
    
    def plot_xgboost_confusion_matrix(self):
        """XGBoost confusion matrix'ini görselleştir."""
        if 'XGBoost' in self.results:
            cm = self.results['XGBoost']['confusion_matrix']
            ...
            plt.ylabel('Gerçek')
            plt.tight_layout()
            plt.savefig('xgb_confusion_matrices.png', dpi=300, bbox_inches='tight')
            plt.show()
    
    def plot_xgboost_roc_curve(self, y_test):
        """XGBoost ROC eğrisini görselleştir."""
        if 'XGBoost' in self.results:
            y_proba = self.results['XGBoost']['y_proba']
            fpr, tpr, _ = roc_curve(y_test, y_proba)
            auc = self.results['XGBoost']['roc_auc']
            ...
            plt.grid(True, alpha=0.3)
            plt.tight_layout()
            plt.savefig('xgb_roc_curves.png', dpi=300, bbox_inches='tight')
            plt.show()
    
    def plot_xgboost_metrics(self):
        """XGBoost metriklerini görselleştir."""
        if 'XGBoost' in self.results:
            metrics = ['Accuracy', 'Precision', 'Recall', 'F1-Score', 'ROC AUC']
            values = [
                ...
                self.results['XGBoost']['roc_auc']
            ]
            
            metrics_df = pd.DataFrame({
                'Model': ['XGBoost'],
                'Accuracy': [self.results['XGBoost']['accuracy']],
                'Precision': [self.results['XGBoost']['precision']],
                'Recall': [self.results['XGBoost']['recall']],
                'F1-Score': [self.results['XGBoost']['f1']],
                'ROC AUC': [self.results['XGBoost']['roc_auc']]
            })
            ...
            for idx, (metric, value) in enumerate(zip(metrics, values)):
                if idx < len(axes):
                    axes[idx].bar(['XGBoost'], [value])
                    axes[idx].set_title(f'XGBoost {metric}')
                    axes[idx].set_ylabel(metric)
                    axes[idx].set_ylim(0, 1)
                    
                    # Değeri çubuğun üzerine yaz
                    axes[idx].text(0, value + 0.01, f'{value:.3f}', ha='center', va='bottom')
    ...
    def plot_xgboost_feature_importance(self, feature_names):
        """XGBoost feature importance'larını görselleştir."""
        ...
        return {'XGBoost': importance_df}
    
    def detailed_xgboost_classification_report(self, y_test):
        """XGBoost detaylı sınıflandırma raporu."""
        if 'XGBoost' in self.results:
            y_pred = self.results['XGBoost']['y_pred']
            print(f"\n=== XGBoost - DETAYLI SINIFLANDIRMA RAPORU ===")
            print(classification_report(y_test, y_pred, target_names=['Kardiyovasküler Yok', 'Kardiyovasküler Var']))
    
    def run_xgboost_analysis(self, X_train, X_test, y_train, y_test, feature_names, data_type=""):
        """XGBoost analiz pipeline'ını çalıştır."""
        print(f"=== KALP KRİZİ RİSK TAHMİN MODELİ - XGBOOST ANALİZ ({data_type.upper()}) ===\n")
        
        # 1. XGBoost modelini eğit
        self.train_xgboost(X_train, y_train)
        
        # 2. XGBoost modelini değerlendir
        self.evaluate_xgboost(X_test, y_test)
        
        # 3. Görselleştirmeler
        print("\nGörselleştirmeler oluşturuluyor...")
        self.plot_xgboost_confusion_matrix()
        self.plot_xgboost_roc_curve(y_test)
        self.plot_xgboost_metrics()
        self.plot_xgboost_feature_importance(feature_names)
        
        # 4. Detaylı rapor
        self.detailed_xgboost_classification_report(y_test)
        
        print(f"\n=== XGBOOST ANALİZ TAMAMLANDI ({data_type.upper()}) ===")
        print(f"En iyi model: XGBoost")
        print(f"En iyi F1-Score: {self.best_score:.4f}")
        
        # Sonuçları döndür
        return {
            'best_model_name': self.best_model_name,
            'best_score': self.best_score,
            'metrics_df': self.plot_xgboost_metrics(),
            'importance_dfs': self.plot_xgboost_feature_importance(feature_names)
        }
