...
def main():
    ...
    parser = argparse.ArgumentParser(description='Kalp Krizi Risk Tahmin Modeli - Karşılaştırmalı Analiz Pipeline')
    parser.add_argument('--use_xgboost', action='store_true', 
                       help='Sadece XGBoost modelini eğit (varsayılan: tüm modeller)')
    
    args = parser.parse_args()
    
    print("="*80)
    if args.use_xgboost:
        print("KALP KRİZİ RİSK TAHMİN MODELİ - XGBOOST ANALİZ PIPELINE")
    else:
    ...
    print("\n" + "="*50)
    if args.use_xgboost:
        print("2. XGBOOST MODEL EĞİTİMİ VE DEĞERLENDİRME")
    else:
    ...
    if args.use_xgboost:
        # Sadece XGBoost için analiz
        outlier_results, no_outlier_results = comparative_analyzer.run_xgboost_only_analysis(
    ...
    print("\n" + "="*80)
    if args.use_xgboost:
        print("FİNAL XGBOOST ANALİZ SONUÇLARI")
    else:
    ...
    # Feature importance (XGBoost için)
    if args.use_xgboost:
        if outlier_results['importance_dfs'] and 'XGBoost' in outlier_results['importance_dfs']:
            print(f"\nEn Önemli 10 Feature - Outlier'lı Veriler (XGBoost):")
            top_features = outlier_results['importance_dfs']['XGBoost'].head(10)
            for idx, row in top_features.iterrows():
                print(f"  {row['Feature']}: {row['Importance']:.4f}")
        
        if no_outlier_results['importance_dfs'] and 'XGBoost' in no_outlier_results['importance_dfs']:
            print(f"\nEn Önemli 10 Feature - Outlier'lar çıkarılmış (XGBoost):")
            top_features = no_outlier_results['importance_dfs']['XGBoost'].head(10)
            for idx, row in top_features.iterrows():
                print(f"  {row['Feature']}: {row['Importance']:.4f}")
    else:
    ...
    print("\n" + "="*80)
    if args.use_xgboost:
        print("XGBOOST ANALİZ PIPELINE TAMAMLANDI!")
        print("Grafikler proje dizinine kaydedildi:")
        print("- xgb_confusion_matrices.png (her iki veri seti için)")
        print("- xgb_roc_curves.png (her iki veri seti için)")
